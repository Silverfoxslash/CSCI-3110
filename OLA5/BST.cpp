...
* Date:	NOV 8 2016
* This file is the implementation of different functions involving a binary search tree.
*/
...
int BinarySearchTree<Comparable>::totalLevel(TreeNode *treePtr, int initLevel) const
{
		...
		{
			int lchild = totalLevel(treePtr->leftChildPtr,initLevel+1);
			int rchild = totalLevel(treePtr->rightChildPtr, initLevel+1);
...
template <typename Comparable>
int BinarySearchTree<Comparable>::totalLevel() const
{
	return totalLevel(root, 0);
}
